...
# weight_mask <- cars$weight < mean(cars$weight)
...
abline(cars_log_heavey_rg, col = "red")
...
cars_log_light_rg_full <- lm( log.mpg.~ log.weight.+ log.acceleration.+ model_year + origin, data=cars_log_light)
summary(cars_log_light_rg_full)
cars_log_heavy_rg_full <- lm( log.mpg.~ log.weight.+ log.acceleration.+ model_year + origin, data=cars_log_heavy)
summary(cars_log_light_rg_full)
...
summary(lm(log.mpg.~log.weight.+log.acceleration.+model_year+factor(origin) + log.weight.*log.acceleration., data=cars_log))
...
interaction_regr <- lm(weight_x_acceleration ~ cars_log$log.weight. + cars_log$log.acceleration.)
...
cars_log2 <- with(cars, data.frame(log(mpg), log(weight),log(cylinders), log(acceleration), model_year, origin))
model1 <- lm(log.weight.~log.cylinders., data = cars_log2)
summary(model1)

### (ii) Model 2: Regress log.mpg. over log.weight. and all control variables

model2 <- lm (log.mpg.~ log.weight.+ log.acceleration.+ model_year+factor(origin)  , data=cars_log2)
summary(model2)
